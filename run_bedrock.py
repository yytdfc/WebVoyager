    ...
    if it == 1:
        init_msg_format = {
            ...
            'content': [
                ...
                {'type': 'text', 'text': f"I've provided the tag name of each element and the text it contains (if text exists). Note that <textarea> or <input> may be textbox, but not exactly. Please focus more on the screenshot and then refer to the textual information.\n{web_text}\n\n{init_msg}"},
            ]
        }
...
def call_claude3_with_bedrock(args, bedrock_client, messages, sys_prompt):
    retry_times = 0
    ...
    input_body["temperature"] = 0.0
    input_body["top_p"] = 0.999
    input_body["top_k"] = 1
    if(len(sys_prompt) > 0):
        input_body["system"] = sys_prompt


    request_body = json.dumps(input_body)
    request_options = {
        "body": request_body,
    ...
    while True:
        try:
            if not args.text_only:
                logging.info('Calling bedrock claude3 API...')

                response = bedrock_client.invoke_model(**request_options)
            ...
            return prompt_tokens, completion_tokens, gpt_call_error, response
...
def main():
    ...
    parser.add_argument("--use_claude3_sonnet", action='store_true')
    ...
    # Build Claude3 with Bedrock Client
    claude_sys_prompt=""
    ...
    for task_id in range(len(tasks)):
        ...
        claude_sys_prompt = SYSTEM_PROMPT_TEXT_ONLY if args.text_only  else  SYSTEM_PROMPT
        obs_prompt =  "Observation: please analyze the accessibility tree and give the Thought and Action." if args.text_only else "Observation: please analyze the attached screenshot and give the Thought and Action. "
        messages = []
        ...
        while it < args.max_iter:
            ...
            #Call Claude3 with Bedrock API
            prompt_tokens, completion_tokens, gpt_call_error, response = call_claude3_with_bedrock(args, boto3_bedrock_client, messages, claude_sys_prompt)
            ...
            res = body_dict['content'][0].get("text")
            ...
            # extract action info
            try:
                assert 'Thought:' in res and 'Action:' in res
            except AssertionError as e:
                logging.error(e)
                fail_obs = "Format ERROR: Both 'Thought' and 'Action' should be included in your reply."
                continue

            # bot_thought = re.split(pattern, res)[1].strip()
            chosen_action = re.split(pattern, res)[2].strip()
            # print(chosen_action)
